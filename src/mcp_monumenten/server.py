...
class MonumentenMCP(FastMCP):  # type: ignore[misc]
    def __init__(
        self,
        name: str = "Monumenten MCP",
        port: int | None = None,
        host: str = "127.0.0.1",
    ...
    def _register_tools(self) -> None:
        ...
        async def get_verblijfsobject_id(
            ...
            postal_code: Annotated[
                Optional[str], "The postal code for search mode 1, e.g. '1234AB'"
            ] = None,
            street: Annotated[
                Optional[str], "The street name for search mode 2, e.g. 'Coolsingel'"
            ] = None,
            city: Annotated[
                Optional[str], "The city name for search mode 2, e.g. 'Rotterdam'"
            ] = None,
            house_letter: Annotated[
                Optional[str], "The house letter, e.g. 'A' in '30A'"
            ] = None,
            house_suffix: Annotated[
                Optional[str], "The house number suffix/addition, e.g. '2' in '30-2'"
            ] = None,
        ) -> str:
            ...
            if search_mode == "postal_code":
                # At this point, postal_code is guaranteed to be non-None
                if postal_code is None:
                    return "Error: postal_code is required for postal code search."
                sparql_query = build_postal_code_query(
                    postal_code, house_number, house_letter, house_suffix
                )
            else:  # address mode
                # At this point, street and city are guaranteed to be non-None
                if street is None or city is None:
                    return "Error: street and city are required for address search."
                    ...
                    async with session.post(
                        ...
                        if response.status == 200:
                            ...
                            if bindings:
                                ...
                                def get_val(b: dict[str, Any], k: str) -> Any:
                                    val = b.get(k, {})
                                ...
                                ids = [
                                    item["bag_verblijfsobject_id"]
                                    for item in full
                                    if item["bag_verblijfsobject_id"]
                                ]
                                if not ids:
                                    if search_mode == "postal_code":
        ...
        async def get_monumental_status(
            bag_verblijfsobject_id: Annotated[
                str, "The verblijfsobject ID (16-18 digits)"
            ],
        ) -> str:
