...
async def test_postal_code_search_coolsingel_30():
    ...
    ids = [
        b.get("identificatie", {}).get("value")
        for b in bindings
        if b.get("identificatie")
    ]
    assert EXPECTED_VERBLIJFSOBJECT_ID in ids, (
        f"Expected ID {EXPECTED_VERBLIJFSOBJECT_ID} not found in results: {ids}"
    )
...
async def test_postal_code_and_address_search_return_same_id():
    ...
    postal_ids = {
        b.get("identificatie", {}).get("value")
        for b in postal_bindings
        if b.get("identificatie")
    }
    address_ids = {
        b.get("identificatie", {}).get("value")
        for b in address_bindings
        if b.get("identificatie")
    }

    assert postal_ids == address_ids, (
        f"Postal code IDs {postal_ids} != Address IDs {address_ids}"
    )
    assert EXPECTED_VERBLIJFSOBJECT_ID in postal_ids, (
        f"Expected ID {EXPECTED_VERBLIJFSOBJECT_ID} not found"
    )
...
async def test_result_contains_address_details():
    ...
    assert target_binding is not None, (
        f"Could not find binding with ID {EXPECTED_VERBLIJFSOBJECT_ID}"
    )
    ...
    assert straatnaam == "Coolsingel", (
        f"Expected straatnaam Coolsingel, got {straatnaam}"
    )
